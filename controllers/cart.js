...
exports.addtocart = async(req,res,next)=>{
    try {
       ...
       res.send({message:'cart quantity updated Successfully',status:'success'});
        }
    } catch (error) {
        console.log(error);
...
exports.AddToCartAndUpdate = async(req,res,next)=>{
    try {
        const user_id = req.body.user_id;
        const product_id = req.body.product_id;
        ...
        data.forEach(doc =>{
            ...
            cartAA.push(cartDetail);
            res.send({message:'cart quantity Add Successfully',status:'success',data:cartAA})
        })
